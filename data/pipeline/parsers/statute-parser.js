...
(function initAtlasStatuteParser(root, factory) {
  ...
  const STRUCTURAL_PATTERNS = {
    en: [
      CORE.pattern("enactment-en", /\b(?:Be\s+it\s+enacted\s+by|ENACTED\s+by)\b/i, -1, "ENACTMENT", () => "ENACTMENT"),
      CORE.pattern("preamble-statute-en", /\b(?:Whereas\s|Preamble|PREAMBLE)\b/i, -1, "PREAMBLE", () => "PREAMBLE"),
    ...
    fr: [
      CORE.pattern("enactment-fr", /\b(?:L'?Assemblée\s+(?:nationale|Nationale)|Le\s+Parlement|Est\s+promulguée)\b/i, -1, "ENACTMENT", () => "ENACTMENT"),
    ...
    de: [
      CORE.pattern("enactment-de", /\b(?:Der\s+Bundestag|Der\s+Reichstag|wird\s+verordnet|Gesetz)\b/i, -1, "ENACTMENT", () => "ENACTMENT"),
    ...
    es: [
      CORE.pattern("enactment-es", /\b(?:Las\s+Cortes\s+Generales\s+han\s+aprobado|se\s+promulga)\b/i, -1, "ENACTMENT", () => "ENACTMENT"),
    ...
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
      ...
      if (trimmed) {
        for (const structurePattern of patterns) {
          ...
          if (["ENACTMENT", "PREAMBLE"].includes(structurePattern.prefix)) continue;
